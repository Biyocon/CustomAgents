...
| F | skrive i EGEN isoleret worktree (i lokal temp uden for repoet), køre checks lokalt inkl. `--apply` i worktree'en efter gate | røre hovedtræet, merge, push, commit uden anmodning, selv køre `git worktree add` |
...
**Fase 5 — Implementering (WS-F, ≤8, kun efter gate).** Pr. godkendt fund-klynge: mindste korrekte rettelse i egen isoleret worktree (i lokal temp uden for repoet), kør relevante checks lokalt (inkl. UTF-8-vagt), producér diff + evidens. Ingen merge/commit uden anmodning.
