...
- **PR C — Adapter plan:** ✅ **LEVERET (2026-07-11).** 7 skema-konforme adapter-beskrivelser i `.agents/model-adapters/` (codex=active, claude-code, kimi, ollama, gemini, cursor, qwen-code) + README + settings-templates pr. runtime. De tidligere manglende **Claude** og **Ollama** er tilføjet. Adaptere validérbare via `validate-schemas.py` (0 overtrædelser). Ingen generatorer (det er PR D/F). Cursor tilføjet som kandidat fra source-library.
- **PR D — Export/validation scripts:** ✅ **LEVERET (2026-07-11).** `.agents/scripts/generate-runtime.py` genererer runtime-output fra canonical pr. aktiv adapter (codex → registry med rige entries + categories/subagents/skills + rolleagent-profiler) til build-dir `.agents/build/runtime/<adapter>/` (gitignoret). `--check` = sync-validation: diff mod live runtime med driftrapport, exit 1 ved drift. Skriver ALDRIG ind i den aktive runtime — aktivering er PR F. Forudsætning leveret samme dag: role-vs-persona afgjort (begge modeller canonical; 19 rolleagenter migreret, commit f0a92088). Kendt drift pr. 2026-07-11 (kvantificeret af `--check`, forventet indtil PR F): 20 agenter kun i canonical (19 roller + council-chairman), 10 arkiverede kun i live, 12 personaer mangler `accent` i canonical (felt-merge-rest, jf. reconciliation trin 2), 36 skills kun i canonical + 1 kun i live (`bdk-forbedringsloop` mangler i `.agents/skills/`). Yderligere script-konsolidering bevidst udskudt (Export-Registry.ps1 forbliver legacy-build indtil PR F deprecates rod-registry).
- **PR E — Memory governance:** ✅ **LEVERET (2026-07-11).** Politik: `docs/architecture/memory-governance.md` — tre klasser (CANONICAL `.agents/brain/` = levende sandhed; RUNTIME-LOKAL `.vscode/.codex/Brain/` = FROSSET transitional; SNAPSHOT `memory/`/`diary/`/`reports/` = append-only). Landing udført: `session-2026-06-17`, `session-history` (runtime-log) og `source-map` landet i canonical med provenance-headere; derefter indeholder runtime-Brain ingen unik varig viden. Frossen-banner på runtime-Brain/AGENTS.md; codex-adapterens `memory_behavior` opdateret. Perplexity/orchestrator-open-decision dispositioneret (memory-delen dækket; orchestrator-spor uden for roadmap A–F). Runtime-Brain-erstatning besluttes ved PR F.
- **PR F — Runtime activation/migration:** ✅ **LEVERET (2026-07-11) — ROADMAP A–F FULDFØRT.** Aktiveret på projektejerens eksplicitte ordre. `generate-runtime.py --apply` skrev genereret registry + 19 rolleagent-profiler ind i `.vscode/.codex/agents/`; håndholdte runtime-filer (14 flade bd-*.md, interface-manager-banebyg.md, 10 afledte undermapper, AGENTS/avatar/skills.yaml i 4 beholdte) afviklet — git-historik bevaret. `--check` = exit 0, reproducerbart (2 kørsler). Loader-test (`invoke-agent.py`): 47 agenter, ingen dubletter. Gate `docs/qa/RELEASE-runtime-activation-gate.md` = **GODKENDT** med evidens + rollback-plan (git revert; canonical urørt af aktiveringen). Ticket #1 lukket → docs/done/. Rod-registry + tom scaffold formelt DEPRECATED. README/AGENTS.md/systemkort opdateret konsistent. Bevidst udskudt (dokumenteret i gaten): fysisk Brain-erstatning (frosset legacy in-place) + evt. `.codex`-rodflytning.
